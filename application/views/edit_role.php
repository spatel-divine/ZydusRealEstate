							...
							<div class="col-md-12">
								...
								<div class="card">
									...
									<div class="card-header pb-0">
										...
										<h3 class="mb-0 card-title">Edit Roles</h3>

									</div>

									<div class="card-body">
										<?php foreach($role1 as $r){ ?>
										 <?php echo form_open('Admin/edit_role_redirect/'.base64_encode($r['admin_id']),array("method"=>"POST")); ?>
										...
										<div class="form-group">
											<div class="row">				
												<div class="col-lg-6 col-md-12">
													...
													<input type="text" class="form-control" name="username" placeholder="Enter User Name" value="<?php echo $r['username'];?>" readonly>
												...
												<div class="col-lg-6 col-md-12">
													...
													<?php if($this->input->post('submit') && $this->input->post('password') !== base64_decode($r['password'])) { ?>
													...
													<?php } else { ?>
														<input type="password" class="form-control" id="password" name="password" placeholder="Enter Password" value="<?php echo base64_decode($r['password']);?>">
													<?php } ?>
										...
										<!--<div class="form-group">
											<div class="row">	
												<div class="col-lg-6 col-md-12">
														<label>Selected Property Group Data<?php $grp_array = explode(",",$r['property_group_id']);?></label><br/>
														<span><?php foreach($property_group as $group){?><?php foreach($grp_array as $grp){ 
															if($grp == $group['property_group_id']){?>
																<b><?php echo $group['property_group'];?>,</b>
															<?php }}}?>
													...
													<div class="col-lg-6 col-md-12">
														<label>Selected Property Group Data Property Access <?php $access = explode(",",$r['property_access']);?></label><br/>
		    												<span><?php foreach($property as $prop){ 	$property_id[] = $prop['property_id'];
																$property_name = $prop['property_name'];	
																$common_access = array_intersect($access,$property_id);?><?php foreach($common_access as $acc){ if($acc == $prop['property_id']){?><b><?php echo $prop['property_name'];?>,</b><?php }}}?></span>
										...
										<div class="form-group">
											<div class="row">				
												<div class="col-lg-4 col-md-12">
													...
													<?php if($this->input->post('submit') && $this->input->post('role_name') !== $r['role_name']) { ?>
													<input type="text" class="form-control" name="role_name" placeholder="Enter Role Name"  value="<?php echo set_value('role_name');?>">
													<?php } else { ?>
													<input type="text" class="form-control" name="role_name" placeholder="Enter Role Name"  value="<?php echo $r['role_name'];?>">
													<?php } ?>
												...
												<?php }} else { $checkarray = explode(",",$r['roles']);}?>

											<div class="col-lg-3 col-md-12">
													<label>Property Group</label>
													<?php $grp_array = explode(",",$r['property_group_id']);?>
													<select name="property_group[]" id="property_group" class="form-control select2" multiple="multiple" style="height:300px">
														
															<?php foreach($property_group as $group){?>
	    													<option value="<?php echo $group['property_group_id'];?>" <?php foreach($grp_array as $grp){  if($grp == $group['property_group_id']){  echo "selected"; }} ?>><?php echo $group['property_group'];?></option>
	    													<?php } ?>
											...
											<div class="col-lg-3 col-md-12">
													<label>Property Access</label>
														...
														<select id="property_name" name="property_name[]" class="form-control" multiple style="height:300px">
															...
															<?php foreach($property as $prop){ 	$property_id[] = $prop['property_id'];
																$property_name = $prop['property_name'];	
																$access = explode(",",$r['property_access']);
																$common_access = array_intersect($access,$property_id);?><option value="<?php echo $prop['property_id'];?>"<?php foreach($common_access as $acc){ if($acc == $prop['property_id']){echo "selected";}}?>><?php echo $prop['property_name'];?></option><?php }?>



														</select>
	                         <input type="hidden" name="proeprty_name_for_edit" id="proeprty_name_for_edit" value="<?php echo $r['property_access']; ?>">
...
$(document).ready(function(){
...

 {
  ...
  <script type="text/javascript">
  		...
  		$('#property_group').on('change',function(){ //debugger
               
 						 var id=$(this).val();
 						 var csrfName = '<?php echo $this->security->get_csrf_token_name(); ?>',
    		     csrfHash = '<?php echo $this->security->get_csrf_hash(); ?>';
    		     if(id && id.length > 0){
	    		     var get_property_for_edit_selection = $("#proeprty_name_for_edit").val();
	    		     var temp = new Array();
							 temp = get_property_for_edit_selection.split(",");
						 }
                         	...
                         	for(i=0; i<data.length; i++){
	                           ...
	                           if(temp && temp.length > 0){
		                           temp.forEach(function(item) {
																   $("#property_name option[value="+item+"]").prop('selected', true);
																});
		                         }
                        } else {
                        	console.log("here");
